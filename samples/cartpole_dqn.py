...
## This is course material for Introduction to Modern Artificial Intelligence
## Example code: cartpole_pid.py
## Citation: OpenAI. ChatGPT (version GPT-4). OpenAI, 2024.
##
## (c) Copyright 2020-2024. Intelligent Racing Inc. Not permitted for commercial use
...
if __name__ == "__main__":
    env = gym.make('CartPole-v1')
    pid = PIDController(kp=20.0, ki=0.0, kd=10.0)
    state_size = env.observation_space.shape[0]
    action_size = env.action_space.n
    done = False
    ...
    for e in range(EPISODES):
        ...
        for time in range(10000):
            ...
            error = pole_angle
            control_signal = pid.compute(error, dt=0.1)  # Assume a fixed time step of 0.02s
            action = 0 if control_signal < 0 else 1  # Apply action based on control signal
